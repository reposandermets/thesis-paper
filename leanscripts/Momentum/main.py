...
class Momentum(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.ROCP = self.ROCP(self.spy.Symbol, self.ROC_PERIOD)

        self.Schedule.On(self.DateRules.MonthStart(),
                         self.TimeRules.At(10, 0), self.MonthlyDeposit)

        self.Schedule.On(self.DateRules.MonthStart(1),
                         self.TimeRules.At(10, 0), self.BuyMomentum)
    ...
    def BuyMomentum(self):
        ...
        self.Plot("ROCP", "ROCP", self.ROCP.Current.Value)
        self.Plot("ROCP", "Mid", 0)

        year = self.Time.year
        t = str(year) + "-" + str(self.Time.month) + \
            "-" + str(self.Time.day)
        if self.ROCP.Current.Value > 0.0:
            self.MarketOrder(self.spy.Symbol, int(buypower))
            self.Debug(t +
                        " ROCP " + str(self.ROCP.Current.Value) +
                        " Buy " + str(int(buypower)))
