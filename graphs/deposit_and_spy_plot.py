...
data_spy = yf.download(['SPY'], start="2000-01-01", end="2023-01-31", interval="1mo")

# Convert the dictionary into a DataFrame
salaries = pd.DataFrame(list(estonian_sal_usd.items()), columns=['Date', 'Salary'])

# Convert the 'Date' column to datetime format and set it as the index
salaries['Date'] = pd.to_datetime(salaries['Date'], format='%Y_%m')
salaries.set_index('Date', inplace=True)

# Make sure data_spy and salaries have the same frequency (monthly)
data_spy_monthly = data_spy['Adj Close'].resample('M').last()

# Create the plot
fig, ax = plt.subplots(figsize=(12, 6))

# Plot the SPY adjusted close
ax.plot(data_spy_monthly.index, data_spy_monthly, label='SPY korrigeeritud sulgemishind', color='blue')

salaries['Salary'] = salaries['Salary'] * 0.1

# Plot the Estonian salaries in USD
ax.plot(salaries.index, salaries['Salary'], label='10% eesti keskmisest palgast', color='green')
